...
class DapDebugProcess(
    ...
    private suspend fun onStopped(event: DapEvent.Stopped) {
        ...
        //
        // lldb-dap can report pause-induced stops with misleading reasons
        // ("exception", "breakpoint", "signal"), so the gate cannot rely on
        // reason alone. It still preserves concrete breakpoint hits by looking
        // at hitBreakpointIds before swallowing the synthetic pause stop.
        val isSynthetic = syntheticPauseGate.consumeIfSynthetic(
            event.payload.reason,
            event.payload.threadId,
            event.payload.hitBreakpointIds,
        )
